...
const CAT_BAR: Record<string, string> = {
  ...
  // legacy
  Education: "bg-sky-400", Agriculture: "bg-lime-500", Housing: "bg-amber-400",
  Health: "bg-rose-400", "Women & Child": "bg-pink-400", "Social Welfare": "bg-teal-500",
  Employment: "bg-violet-500", "Business & MSME": "bg-orange-400", Pension: "bg-yellow-500",
  Scholarship: "bg-indigo-400", "Skill Development": "bg-cyan-500",
  "Minority Welfare": "bg-purple-400", "Differently Abled": "bg-green-500",
  "Financial Inclusion": "bg-blue-500",
};

const CAT_ICON_BG: Record<string, string> = {
  "Agriculture,Rural & Environment":           "bg-lime-50   text-lime-600",
  "Banking,Financial Services and Insurance":  "bg-blue-50   text-blue-600",
  "Business & Entrepreneurship":               "bg-orange-50 text-orange-600",
  "Education & Learning":                      "bg-sky-50    text-sky-600",
  "Health & Wellness":                         "bg-rose-50   text-rose-500",
  "Housing & Shelter":                         "bg-amber-50  text-amber-600",
  "Public Safety,Law & Justice":               "bg-slate-100 text-slate-600",
  "Science, IT & Communications":              "bg-cyan-50   text-cyan-600",
  "Skills & Employment":                       "bg-violet-50 text-violet-600",
  "Social welfare & Empowerment":              "bg-teal-50   text-teal-600",
  "Sports & Culture":                          "bg-pink-50   text-pink-500",
  "Transport & Infrastructure":                "bg-indigo-50 text-indigo-600",
  "Travel & Tourism":                          "bg-emerald-50 text-emerald-600",
  "Utility & Sanitation":                      "bg-yellow-50 text-yellow-600",
  "Women and Child":                           "bg-fuchsia-50 text-fuchsia-500",
};
...
export default function SchemeCard({ scheme, showEligibility = false, compact = false }: Props) {
  ...
  const iconBg     = CAT_ICON_BG[primaryCat] ?? CAT_ICON_BG[scheme.scheme_category] ?? "bg-slate-100 text-slate-500";
      ...
      <article className={cn(
        ...
        <div className={cn("flex flex-col flex-1", compact ? "p-4" : "p-5")}>
          ...
          <div className="flex items-start gap-3 mb-3">
            {/* SVG icon box */}
            <div className={cn(
              "rounded-xl flex items-center justify-center shrink-0 border border-slate-100",
              iconBg,
              compact ? "w-10 h-10" : "w-11 h-11"
            )}>
              <CategoryIcon category={scheme.scheme_category} size={compact ? 18 : 20} strokeWidth={1.8} />
            </div>
            ...
            <div className="min-w-0 flex-1 overflow-hidden">
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                ...
                <span className="text-[11px] text-slate-500 truncate">{catLabel}</span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-2">
            ...
            <ArrowRight size={14} className="text-slate-300 group-hover:text-slate-700 group-hover:translate-x-0.5 transition-all shrink-0" />
          </div>
        </div>
      </article>
    </Link>
